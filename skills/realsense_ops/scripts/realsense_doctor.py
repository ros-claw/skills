...
def usb_link() -> list[dict]:
    """Locate RealSense devices (vendor 8086) in sysfs and report link speed.

    A camera on a 480M (USB2) bus is the silent bandwidth trap: streams
    start but drop frames, high res/FPS never delivers — move the cable,
    don't debug the app."""
    rows = []
    for path in glob.glob("/sys/bus/usb/devices/*"):
        try:
            vendor = open(f"{path}/idVendor").read().strip()
        except OSError:
            continue
        if vendor != "8086":
            continue
        try:
            product = open(f"{path}/idProduct").read().strip()
            speed = open(f"{path}/speed").read().strip()
            busnum = open(f"{path}/busnum").read().strip()
            devname = open(f"{path}/product").read().strip()
        except OSError:
            continue
        rows.append({
            "device": devname, "vid_pid": f"{vendor}:{product}",
            "bus": f"usb{busnum}", "speed_mbps": speed,
            "usb2_fallback": speed == "480",
        })
    return rows
...
def main() -> int:
    ...
    for link in report["usb_link"]:
        if link.get("usb2_fallback"):
            rec.append(f"{link['device']} is on a USB2 (480M) bus {link['bus']} — move it to a SuperSpeed port before any other debugging.")
